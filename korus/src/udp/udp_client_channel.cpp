...
int32_t	udp_client_handler_base::send(const std::shared_ptr<buffer_thunk>& data, const sockaddr_in& peer_addr)
{
	if (!_tunnel_prev)
	{
		assert(false);
...
int32_t	udp_client_handler_base::send(const std::shared_ptr<buffer_thunk>& data)
{
	if (!_tunnel_prev)
	{
		assert(false);
...
int32_t	udp_client_handler_origin::send(const std::shared_ptr<buffer_thunk>& data, const sockaddr_in& peer_addr)
{
	if (!is_normal())
	{
		assert(false);
...
int32_t	udp_client_handler_origin::send(const std::shared_ptr<buffer_thunk>& data)								// 保证原子, 认为是整包，返回值若<0参考CHANNEL_ERROR_CODE
{
	if (!is_normal())
	{
		assert(false);
...
int32_t	udp_client_handler_origin::on_recv_buff(const std::shared_ptr<buffer_thunk>& data, const sockaddr_in& peer_addr)
{
	if (!is_normal())
	{
		assert(false);
...
int32_t	udp_client_handler_terminal::send(const void* buf, const size_t len, const sockaddr_in& peer_addr)
{
	...
	return udp_client_handler_terminal::send(thunk, peer_addr);
}
...
{
	std::shared_ptr<buffer_thunk>	thunk = std::make_shared<buffer_thunk>(buf, len);
	return udp_client_handler_terminal::send(thunk);
}
...
int32_t	udp_client_handler_terminal::send(const std::shared_ptr<buffer_thunk>& data, const sockaddr_in& peer_addr)
{
	if (!reactor()->is_current_thread())
	{
		reactor()->start_async_task(std::bind(static_cast<void (udp_client_handler_terminal::*)(const std::shared_ptr<buffer_thunk>&, const sockaddr_in&)>(&udp_client_handler_terminal::send_async), this, data, peer_addr), this);
		return data->used();
	}
	return udp_client_handler_base::send(data, peer_addr);
}

int32_t	udp_client_handler_terminal::send(const std::shared_ptr<buffer_thunk>& data)
{
	if (!reactor()->is_current_thread())
	{
		reactor()->start_async_task(std::bind(static_cast<void (udp_client_handler_terminal::*)(const std::shared_ptr<buffer_thunk>&)>(&udp_client_handler_terminal::send_async), this, data), this);
		return data->used();
	}
	return udp_client_handler_base::send(data);
}

void	udp_client_handler_terminal::send_async(const std::shared_ptr<buffer_thunk>& data, const sockaddr_in& peer_addr)
{
	int32_t ret = udp_client_handler_base::send(data, peer_addr);
	if (ret < 0)
	{
...
void	udp_client_handler_terminal::send_async(const std::shared_ptr<buffer_thunk>& data)
{
	int32_t ret = udp_client_handler_base::send(data);
	if (ret < 0)
	{
		udp_client_handler_base::close();
	}
