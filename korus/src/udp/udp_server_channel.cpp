...
//这是一个待处理的完整包
void	udp_server_handler_base::on_recv_pkg(const std::shared_ptr<buffer_thunk>& data, const sockaddr_in& peer_addr)
{
	if (!_tunnel_next)
	{
		assert(false);
...
void	udp_server_handler_terminal::close()
{
	if (!reactor()->is_current_thread())
	{
		reactor()->start_async_task(std::bind(&udp_server_handler_terminal::close, this), this);
		return;
	}
	udp_server_handler_base::close();
}
