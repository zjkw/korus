...
{
public:
	socks5_server(uint16_t thread_num, const std::string& tcp_listen_addr, const std::string& udp_listen_ip, const std::shared_ptr<socks5_server_auth> auth, uint32_t backlog = DEFAULT_LISTEN_BACKLOG, uint32_t defer_accept = DEFAULT_DEFER_ACCEPT,
		...
		: _udp_listen_ip(udp_listen_ip), _auth(auth),
...
private:
	...
	{
		socks5_server_init_channel* channel = new socks5_server_init_channel(reactor, _udp_listen_ip, _auth);
...
public:
	// addr格式ip:port
	socks5_server(std::shared_ptr<reactor_loop> reactor, const std::string& tcp_listen_addr, const std::string& udp_listen_ip, const std::shared_ptr<socks5_server_auth> auth, uint32_t backlog = DEFAULT_LISTEN_BACKLOG, uint32_t defer_accept = DEFAULT_DEFER_ACCEPT,
		const uint32_t self_read_size = DEFAULT_READ_BUFSIZE, const uint32_t self_write_size = DEFAULT_WRITE_BUFSIZE, const uint32_t sock_read_size = 0, const uint32_t sock_write_size = 0)
		: _udp_listen_ip(udp_listen_ip), _auth(auth),
