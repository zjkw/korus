...
int32_t socks5_bindcmd_filterbind_client_channel::on_recv_split(const std::shared_ptr<buffer_thunk>& data)
{
	return _integration->on_data_recv_split(data);
}

void	socks5_bindcmd_filterbind_client_channel::on_recv_pkg(const std::shared_ptr<buffer_thunk>& data)
{
	_integration->on_data_recv_pkg(data);
}
